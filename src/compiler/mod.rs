...
impl<'h> Compiler<'h> {
	pub fn compile(source: String, heap: &'h mut Heap) -> Result<Chunk, ()> {
		let mut compiler = Compiler::init(source.chars().collect(), heap);
	...
	fn init(source: Vec<char>, heap: &'h mut Heap) -> Compiler {
		Compiler {
